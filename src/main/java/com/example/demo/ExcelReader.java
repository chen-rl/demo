...
//    private static Logger logger = Logger.getLogger(ExcelReader.class.getName()); // 日志打印类
    ...
    private static ExcelDataVO convertRowToData(Row row) {
        ...
        // 获取v6id
        cell = row.getCell(cellNum++);
        String v6id = convertCellValueToString(cell);
        if (null == v6id || "".equals(v6id)) {
            // v6id为空
            resultData.setV6_id(null);
        } else {
            resultData.setV6_id(Integer.parseInt(v6id));
        }
        // 获取ipv6子网
        cell = row.getCell(cellNum++);
        String v6_website = convertCellValueToString(cell);
        resultData.setV6_website(v6_website);
        // 获取v6id
        cell = row.getCell(cellNum++);
        String v4id = convertCellValueToString(cell);
        if (null == v4id || "".equals(v4id)) {
            // v4id为空
            resultData.setV4_id(null);
        } else {
            resultData.setV4_id(Integer.parseInt(v4id));
        }
        // 获取ipv4子网
        cell = row.getCell(cellNum++);
        String v4_website = convertCellValueToString(cell);
        resultData.setV4_website(v4_website);
        // 获取vlan
        cell = row.getCell(cellNum++);
        String vlan = convertCellValueToString(cell);
        if (null == vlan || "".equals(vlan)) {
            // v4id为空
            resultData.setVlan(null);
        } else {
            resultData.setVlan(Integer.parseInt(vlan));
        }
